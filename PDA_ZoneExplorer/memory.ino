...
void addEvent(int id, String eventName, int hour, int min, int date, int month, int year,
  ...
  if (hour < 0 || hour > 23 || min < 0 || min > 59 || date < 1 || date > 31 || month < 1 || month > 12 || year < 2000 || duration < 1 || duration > 240) {
    Serial2Webln("Помилка: Невірна дата, час або тривалість (1-240 хв).");
    return;
  }

  // Проверка валидности значений уведомлений
  if ((notify2h != 0 && notify2h != 1) || (notify1h != 0 && notify1h != 1) || (notify30m != 0 && notify30m != 1) || (notify10m != 0 && notify10m != 1) || (notify1m != 0 && notify1m != 1)) {
